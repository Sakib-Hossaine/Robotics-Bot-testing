...
void handlePauseWithMoistureCheck() {
  ...
  if (checkingMoisture) {
    ...
    if (!servoDown) {
      if (currentTime - moistureCheckStartTime >= 200) {  // Small delay before lowering
    ...
    // Step 2: Read soil moisture (after servo is down)
      ...
      if (currentTime - moistureCheckStartTime >= 500) {  // Wait for servo to settle
        int soilMoisture = readSoilMoisture();
        Serial.print("Soil Moisture Value: ");
        Serial.println(soilMoisture);
        
        if (soilMoisture > DRY_THRESHOLD) {
          Serial.println("Soil is DRY - Activating pump for 2 seconds");
          activatePump();
